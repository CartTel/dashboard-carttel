...
type ArrivalShipmentProps = {
    onClose: () => void; // Function to close the modal
    id: number; // ID of the request to approve
    // slaId: number;
};

export function ArrivalShipmentRequest({ onClose, id}: ArrivalShipmentProps) {
    ...
    const handleArrivalShipment = async (e: React.FormEvent<HTMLFormElement>) => {
        e?.preventDefault();
        setLoading(true);
        setError("");
        setSuccessMessage("");
    ...
    return (
        <form
            onSubmit={handleArrivalShipment}
            ...
            <div className="text-center w-full">
                <SecondaryText className="mb-[23px]">Arrival At The Warehouse</SecondaryText>
            </div>
                    ...
                    <div>
                        {loading ? ( // Display spinner if userLoading is true
                            ...
                            <div className='flex items-center justify-center text-[13px]'>
                                ...
                                <span className="ml-0">Arrival shipment</span>
                            </div>
                        )}
