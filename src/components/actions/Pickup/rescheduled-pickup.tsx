...
import { rescheduledPickupRequest } from "@/config/api";
import { useRouter } from "next/navigation";
import { toast } from "@/hooks/use-toast";



type ApprovePickupProps = {
...
export function RescheduledPickup({ onClose, id }: ApprovePickupProps) {
    ...
    const handleRescheduledPickup = async (e: React.FormEvent<HTMLFormElement>) => {
        e?.preventDefault();
        setLoading(true);
        console.log("object food", formData);
        try {
            const data = await rescheduledPickupRequest(formData?.pickupTime, formData?.user_id, formData?.pickup_id, formData?.pickupDate);
    ...
    return (
        <form
            ...
            <div className="text-center w-full">
                <SecondaryText className="mb-[23px] !text-[16px]">Rescheduled Pickup</SecondaryText>
            </div>
                ...
                <div>
                    <div className=''>
                        ...
                        <div className="group relative mt-2 md:!w-full xs:w-full flex justify-center items-center ">
                            <div className="gap-10 md:h-[15vh] xs:h-[15vh] md:!w-[500px] xs:w-full flex justify-center items-center">
            ...
            <div className="flex items-center justify-center gap-[20px] lg:flex-row xs:flex-col w-full lg:w-auto mt-10">
                ...
                <CustomButton
                    ...
                    className={`${loading || !arePickupItems ? "!bg-[#F47F12] opacity-40 text-white" : "!bg-[#F47F12] text-white"}  !py-[0px] !h-[58px] lg:!w-[175px] xs:!w-[300px] !text-md`}
                >
                    <div>
                        {loading ? ( // Display spinner if userLoading is true
                            ...
                            <div className='flex items-center justify-center text-[13px]'>
                                ...
                                <span className="ml-0">Reschedule</span>
                            </div>
                        )}
