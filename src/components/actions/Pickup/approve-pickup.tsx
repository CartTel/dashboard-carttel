...
import { approveShipment, approvedPickupRequest } from "@/config/api";
...
export function ApprovePickup({ onClose, id }: ApprovePickupProps) {
    ...
    const handleApprovePickup = async (e: React.FormEvent<HTMLFormElement>) => {
        e?.preventDefault();
        setLoading(true);
        setError("");
        setSuccessMessage("");
        ...
        try {
            ...
            const result = await approvedPickupRequest(id);
            if (result) {
                toast({
                    title: "Success",
                    description: `Approve Pickup Successfully 🎉`,
                    variant: "destructive",
                });
            ...
            // Handle success
            setSuccessMessage("Approve Pickup Successfully!");
        } catch (err: any) {
    ...
    return (
        <form
            ...
            <div className="text-center w-full">
                <SecondaryText className="mb-[23px]">Approve Pickup</SecondaryText>
            </div>
                    ...
                    <div>
                        {loading ? ( // Display spinner if userLoading is true
                            ...
                            <div className='flex items-center justify-center text-[13px]'>
                                ...
                                <span className="ml-0">Approve Pickup</span>
                            </div>
                        )}
