...
import { CompletedShipment, RestartProcessRequest, ConfirmProcessRequest } from "@/config/api";
import { useRouter } from "next/navigation";
import { toast } from "@/hooks/use-toast";



type ConfirmProcessProps = {
    onClose: () => void; // Function to close the modal
    id: number; // ID of the request to approve
    // slaId: number;
};

export function ConfirmProcess({ onClose, id}: ConfirmProcessProps) {
    ...
    const handleConfirmProcess = async (e: React.FormEvent<HTMLFormElement>) => {
        e?.preventDefault();
        setLoading(true);
        setError("");
        setSuccessMessage("");
        ...
        try {
            ...
            const result = await ConfirmProcessRequest(id);
            if (result) {
                toast({
                    title: "Success",
                    description: `Confirm Procurement Successfully 🎉`,
                    variant: "destructive",
                });
            ...
            setSuccessMessage("Confirm Procurement Successfully!");
        } catch (err: any) {
    ...
    return (
        <form
            ...
            <div className="text-center w-full">
                <SecondaryText className="mb-[23px]">Confirm Request</SecondaryText>
            </div>
                    ...
                    <div>
                        {loading ? ( // Display spinner if userLoading is true
                            ...
                            <div className='flex items-center justify-center text-[13px]'>
                                ...
                                <span className="ml-0">Confirm Request</span>
                            </div>
                        )}
