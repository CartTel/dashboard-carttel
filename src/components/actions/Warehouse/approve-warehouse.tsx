...
import { approvedWarehouseRequest } from "@/config/api";
import { useRouter } from "next/navigation";
import { toast } from "@/hooks/use-toast";



type ApproveWarehouseProps = {
    onClose: () => void; // Function to close the modal
    id: number; // ID of the request to approve
};

export function ApproveWarehouse({ onClose, id }: ApproveWarehouseProps) {
    ...
    const handleApproveWarehouse = async (e: React.FormEvent<HTMLFormElement>) => {
        e?.preventDefault();
        setLoading(true);
        setError("");
        setSuccessMessage("");
        ...
        try {
            ...
            const result = await approvedWarehouseRequest(id);
            if (result) {
                toast({
                    title: "Success",
                    description: `Approve Warehouse Successfully 🎉`,
                    variant: "destructive",
                });
            ...
            // Handle success
            setSuccessMessage("Approve Warehouse Successfully!");
        } catch (err: any) {
    ...
    return (
        <form
            onSubmit={handleApproveWarehouse}
            ...
            <div className="text-center w-full">
                <SecondaryText className="mb-[23px]">Approve Warehouse</SecondaryText>
            </div>
                    ...
                    <div>
                        {loading ? ( // Display spinner if userLoading is true
                            ...
                            <div className='flex items-center justify-center text-[13px]'>
                                ...
                                <span className="ml-0">Approve Warehouse</span>
                            </div>
                        )}
                    </div>
                </CustomButton>
