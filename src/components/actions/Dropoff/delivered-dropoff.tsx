...
import { deliveredDropoffRequest } from "@/config/api";
import { useRouter } from "next/navigation";
import { toast } from "@/hooks/use-toast";



type DeliveredDropoffProps = {
    onClose: () => void; // Function to close the modal
    id: number; // ID of the request to approve
};

export function DeliveredDropoff({ onClose, id }: DeliveredDropoffProps) {
    ...
    const handleDeliveredDropoff = async (e: React.FormEvent<HTMLFormElement>) => {
        e?.preventDefault();
        setLoading(true);
        setError("");
        setSuccessMessage("");
        ...
        try {
            ...
            const result = await deliveredDropoffRequest(id);
            if (result) {
                toast({
                    title: "Success",
                    description: `Dropoff Delivered Successfully 🎉`,
                    variant: "destructive",
                });
            ...
            // Handle success
            setSuccessMessage("Dropoff Delievered Successfully!");
        } catch (err: any) {
    ...
    return (
        <form
            onSubmit={handleDeliveredDropoff}
            ...
            <div className="text-center w-full">
                <SecondaryText className="mb-[23px]">Delivered Dropoff</SecondaryText>
            </div>
                    ...
                    <div>
                        {loading ? ( // Display spinner if userLoading is true
                            ...
                            <div className='flex items-center justify-center text-[13px]'>
                                ...
                                <span className="ml-0">Delivered Dropoff</span>
                            </div>
                        )}
                    </div>
                </CustomButton>
