...
import { intransitDropoffRequest } from "@/config/api";
import { useRouter } from "next/navigation";
import { toast } from "@/hooks/use-toast";



type IntransitDropoffProps = {
    onClose: () => void; // Function to close the modal
    id: number; // ID of the request to approve
};

export function IntransitDropoff({ onClose, id }: IntransitDropoffProps) {
    ...
    const handleIntransitDropoff = async (e: React.FormEvent<HTMLFormElement>) => {
        e?.preventDefault();
        setLoading(true);
        setError("");
        setSuccessMessage("");
        ...
        try {
            ...
            const result = await intransitDropoffRequest(id);
            if (result) {
                toast({
                    title: "Success",
                    description: `Dropoff Intransit Successfully 🎉`,
                    variant: "destructive",
                });
            ...
            // Handle success
            setSuccessMessage("Dropoff Intransit Successfully!");
        } catch (err: any) {
    ...
    return (
        <form
            onSubmit={handleIntransitDropoff}
            ...
            <div className="text-center w-full">
                <SecondaryText className="mb-[23px]">Intransit Dropoff</SecondaryText>
            </div>
            ...
            <div className="flex items-center justify-center gap-[20px] lg:flex-row xs:flex-col w-full lg:w-auto">
                ...
                <CustomButton
                    disabled={loading}
                    // loaderState={loading}
                    className={`!bg-purple-500 !py-[0px] !h-[58px] lg:!w-[175px] xs:!w-[300px] !text-md`}
                >
                    <div>
                        {loading ? ( // Display spinner if userLoading is true
                            ...
                            <div className='flex items-center justify-center text-[13px]'>
                                ...
                                <span className="ml-0">Intranist Dropoff</span>
                            </div>
                        )}
                    </div>
                </CustomButton>
