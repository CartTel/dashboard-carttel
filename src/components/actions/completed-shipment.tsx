...
import { CompletedShipment } from "@/config/api";
import { useRouter } from "next/navigation";
import { toast } from "@/hooks/use-toast";



type CompletedShipmentProps = {
    onClose: () => void; // Function to close the modal
    id: number; // ID of the request to approve
    // slaId: number;
};

export function CompletedShipmentRequest({ onClose, id}: CompletedShipmentProps) {
    ...
    const handleCompletedShipment = async (e: React.FormEvent<HTMLFormElement>) => {
        e?.preventDefault();
        setLoading(true);
        setError("");
        setSuccessMessage("");
    ...
    return (
        <form
            onSubmit={handleCompletedShipment}
            ...
            <div className="text-center w-full">
                <SecondaryText className="mb-[23px]">Shipment Complete</SecondaryText>
            </div>
            ...
            <div className="flex items-center justify-center gap-[20px] lg:flex-row xs:flex-col w-full lg:w-auto">
                ...
                <CustomButton
                    disabled={loading}
                    // loaderState={loading}
                    className={`!bg-lime-500 !py-[0px] !h-[58px] lg:!w-[175px] xs:!w-[300px] !text-md`}
                >
                    <div>
                        {loading ? ( // Display spinner if userLoading is true
                            ...
                            <div className='flex items-center justify-center text-[13px]'>
                                ...
                                <span className="ml-0">Complete Shipment</span>
                            </div>
                        )}
