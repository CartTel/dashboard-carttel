...
import { IntransitShipment } from "@/config/api";
import { useRouter } from "next/navigation";
import { toast } from "@/hooks/use-toast";



type IntransitShipmentProps = {
    onClose: () => void; // Function to close the modal
    id: number; // ID of the request to approve
    // slaId: number;
};

export function IntransitShipmentRequest({ onClose, id}: IntransitShipmentProps) {
    ...
    const handleIntransitShipment = async (e: React.FormEvent<HTMLFormElement>) => {
        e?.preventDefault();
        setLoading(true);
        setError("");
        setSuccessMessage("");
    ...
    return (
        <form
            onSubmit={handleIntransitShipment}
            ...
            <div className="text-center w-full">
                <SecondaryText className="mb-[23px]">Shipment Intransit</SecondaryText>
            </div>
                    ...
                    <div>
                        {loading ? ( // Display spinner if userLoading is true
                            ...
                            <div className='flex items-center justify-center text-[13px]'>
                                ...
                                <span className="ml-0">Intransit shipment</span>
                            </div>
                        )}
