import { IIcon } from "@/libs/interfaces";
import React from "react";

export function DropoffFilledIcon({
    fillColor = "#103f69",
    size = 24,
    className,
}: IIcon) {
    return (
        <svg
            className={className}
            width={size}
            height={size}
            viewBox="0 0 24 24"
            fill="none"
            xmlns="http://www.w3.org/2000/svg"
        >
            <path
                d="M14.001 2.92V11.23C14.001 12.25 13.171 13.08 12.151 13.08H3.00098C2.45098 13.08 2.00098 12.63 2.00098 12.08V5.69C2.00098 3.65 3.65098 2 5.69098 2H13.071C13.591 2 14.001 2.41 14.001 2.92Z"
                fill={fillColor}
            />
            <path
                d="M21.501 15.5C21.781 15.5 22.001 15.72 22.001 16V17C22.001 18.66 20.661 20 19.001 20C19.001 18.35 17.651 17 16.001 17C14.351 17 13.001 18.35 13.001 20H11.001C11.001 18.35 9.65098 17 8.00098 17C6.35098 17 5.00098 18.35 5.00098 20C3.34098 20 2.00098 18.66 2.00098 17V15C2.00098 14.45 2.45098 14 3.00098 14H12.501C13.881 14 15.001 12.88 15.001 11.5V6C15.001 5.45 15.451 5 16.001 5H16.841C17.561 5 18.221 5.39 18.581 6.01L19.221 7.13C19.311 7.29 19.191 7.5 19.001 7.5C17.621 7.5 16.501 8.62 16.501 10V13C16.501 14.38 17.621 15.5 19.001 15.5H21.501Z"
                fill={fillColor}
            />
            <path
                d="M8.00098 22C9.10555 22 10.001 21.1046 10.001 20C10.001 18.8954 9.10555 18 8.00098 18C6.89641 18 6.00098 18.8954 6.00098 20C6.00098 21.1046 6.89641 22 8.00098 22Z"
                fill={fillColor}
            />
            <path
                d="M16.001 22C17.1055 22 18.001 21.1046 18.001 20C18.001 18.8954 17.1055 18 16.001 18C14.8964 18 14.001 18.8954 14.001 20C14.001 21.1046 14.8964 22 16.001 22Z"
                fill={fillColor}
            />
            <path
                d="M22.001 12.53V14H19.001C18.451 14 18.001 13.55 18.001 13V10C18.001 9.45 18.451 9 19.001 9H20.291L21.741 11.54C21.911 11.84 22.001 12.18 22.001 12.53Z"
                fill={fillColor}
            />
        </svg>
    );
}
